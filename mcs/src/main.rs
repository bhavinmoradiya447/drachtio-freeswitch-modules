...
async fn main() {
    ...
    // create a counter for number of requests
    let _counter = Arc::new(Mutex::new(0));
    let with_counter = warp::any().map(move || Arc::clone(&_counter));
...
fn parse_header(buf: &[u8], start_pos: usize) -> Result<Payload, Error> {
    ...
    payload.uuid = Uuid::from_slice(&buf[start_pos..start_pos + 16])
        .unwrap()
        .to_string();
...
fn parse_payload(buf: &[u8], start_pos: usize, end_pos: usize) -> (Payload, usize) {
    ...
    trace!(
        "parsed payload: uuid: {}, seq: {}, size: {}, new_pos: {}",
        payload.uuid,
        payload.seq,
        payload.size,
        new_pos
