...
impl AddressChannel {
    ...
    async fn new(
        ...
        {
            let uuid_client_channel = UuidClientChannel::new(address.clone(), client.clone());
...
async fn main() -> Result<(), Error> {
    ...
    loop {
        ...
        if size != 0 {
            ...
            let _ = fd.read_exact(&mut payload_buf);
...
async fn start_cast_handler(
    ...
    if address_to_channel.contains_key(&address) {
        ...
        address_channel.uuid_client_channels.lock().await.insert(uuid.clone(), UuidClientChannel::new(address.clone(), address_channel.client.clone()));
    } else {
